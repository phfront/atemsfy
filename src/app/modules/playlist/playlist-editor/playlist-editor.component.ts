...
export class PlaylistEditorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.breakpointObserver
      .observe([`(max-width: ${this.mobile_breaker})`])
  ...
  filterPlaylist(playlist) {
    ...
    if (this.playlists_filter.text !== '') {
      return (
        playlist.name
          .toLowerCase()
          .indexOf(this.playlists_filter.text.toLowerCase()) > -1
      );
    }
    return true;
  }
  ...
  getPlaylistTracks(url) {
    this.spotifyService
      .get(url, { playlist_id: this.playlist.id }, { limit: 50 })
      .subscribe(
        (response) => {
          this.playlists_tracks = this.playlists_tracks.filter(track => {
            if (track) {
              return track;
            }
          }).concat(
  ...
  initSearch() {
    this.available_tracks_search.observer
    ...
    return this.playlists_tracks.filter((track) => {
      if (track) {
        if (
          track.name
            .toLowerCase()
            .indexOf(this.playlists_tracks_search.value.toLowerCase()) > -1
        ) {
          return track;
        }
      }
  ...
  drop(event: CdkDragDrop<string[]>) {
    if (event.previousContainer === event.container) {
      ...
      if (
        event.container.element.nativeElement.getAttribute(
          'playlist-tracks'
        ) === 'true'
      ) {
        if (event.previousIndex !== event.currentIndex) {
          if (event.previousIndex < event.currentIndex) {
            this.reorderPlaylistTracks(
              event.previousIndex,
              1,
      ...
      this.addTrackToPlaystist(
        this.playlists_tracks[event.currentIndex],
        event.currentIndex
      );
      this.available_tracks.splice(
        event.previousIndex,
        0,
        this.playlists_tracks[event.currentIndex]
      );
      ...
      .subscribe(
        (response) => {
          this.messageService.success(
            `Música ${track.name} adicionada na playlist ${this.playlist.name}`,
            {
      ...
      this.spotifyService
        ...
        .subscribe(
          (response) => {
            ...
            this.playlists_tracks.splice(index, 1);
            this.messageService.success(
              `Música ${track.name} removida da playlist ${this.playlist.name}`,
              {
                position: this.message_position,
              }
            );
  ...
  reorderPlaylistTracks(range_start, range_length, insert_before) {
    ...
    this.spotifyService
      ...
      .subscribe(
        (response) => {
          this.messageService.success(
            `A ordem das música da playlist ${this.playlist.name} foi alterada`,
            {
              position: this.message_position,
            }
          );
    ...
    if (playlist_name !== null && playlist_name !== '') {
      this.spotifyService
        ...
        .subscribe(
          (response) => {
            this.messageService.success(
              `Playlist ${playlist_name} foi adicionada`,
              {
                position: this.message_position,
              }
            );
  ...
  selectTrack(track) {
    const index = this.available_tracks_search.selected.indexOf(track);
    if (index === -1) {
    ...
    if (save) {
      ...
      this.spotifyService
        .post(
          ...
          {
            uris: this.available_tracks_search.selected.map(
              (track) => track.uri
            ),
          }
        )
        .subscribe(
          (response) => {
            this.messageService.success(
              `Músicas adicionadas na playlist ${this.playlist.name}`,
              {
                position: this.message_position,
              }
            );
